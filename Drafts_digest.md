...
- Period-subspace DS: when multiple images are available per side, stack spectra (or deep features such as ResNet encodings) into “before” and “after” period subspaces and apply DS between them (e.g., first 15 vs last 15 days of a month, or first 6 vs last 6 months of a year for a given location) to reduce single-date noise/cloud sensitivity and capture temporal regime shifts (period definition and scope TBD).
