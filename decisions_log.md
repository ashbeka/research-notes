...
Context: Senpai feedback emphasized having one clear task ("basically classifying the damage of an area") and user constraints limit how much can be built in one master’s project. Sensei also recommended using subspace representation (DS) as an initial tool before more advanced hybrids. The repo already plans a long-term hybrid DS + SSC + U-Net + MCDA pipeline, plus future directions (land-use, infra planning, GANs, LLMs, etc.); provenance: notes/senpais_notes.md, notes/sensei_notes.md, and notes/old_notes.md.
...
## 0016 — Notes Repo vs Implementation Repo Separation
